...
export default function DashboardPage() {
  ...
  const contexts=useMemo(()=>buildIssueContexts(runs),[runs]);
  const openRoots=useMemo(()=>groupRootIssues(issues.filter(i=>i.status==='open'),contexts),[issues,contexts]);
  const activeRun=useMemo(()=>runs.find(r=>r.status==='running')||null,[runs]);
  const activeWorkers=useMemo(()=>runs.reduce((count,run)=>count+(run.browserSessions?.filter(session=>session.status==='active').length||0),0),[runs]);
  const metrics=useMemo(()=>({total:runs.length,running:runs.filter(r=>r.status==='running').length,passed:runs.filter(r=>r.status==='passed'||r.status==='completed').length,failed:runs.filter(r=>['failed','error','aborted'].includes(r.status)).length,open:openRoots.length}),[runs,openRoots]);
  ...
  return <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-150">
    ...
    <AutomationStatusCard activeRun={activeRun} activeWorkers={activeWorkers}/>
    <div className="grid lg:grid-cols-3 gap-6"><Card className="lg:col-span-2 p-5"><div className="flex items-center justify-between mb-4"><h2 className="font-semibold">Recent Runs</h2><Link href="/runs" className="text-xs text-primary">View all</Link></div>{runs.length===0?<p className="text-sm text-muted-foreground py-8 text-center">No runs yet.</p>:<div className="space-y-2">{runs.slice(0,6).map(r=><Link href={`/runs/${r.id}`} key={r.id} className="flex items-center justify-between gap-3 border border-border rounded-lg p-3 hover:border-primary/30"><div className="min-w-0"><p className="font-mono text-xs truncate">{r.id}</p><p className="text-xs text-muted-foreground mt-1">{r.task?.project?.name || r.task?.targetUrl || 'Project context unavailable'} · {formatDate(r.startedAt || r.createdAt)}</p></div><Badge variant={r.status==='passed'||r.status==='completed'?'success':r.status==='running'?'info':['failed','error','aborted'].includes(r.status)?'danger':'neutral'}>{r.status}</Badge></Link>)}</div>}</Card><Card className="p-5"><div className="flex items-center justify-between mb-4"><h2 className="font-semibold">Open Issues</h2><Link href="/issues" className="text-xs text-primary">View all</Link></div>{openRoots.length===0?<p className="text-sm text-muted-foreground py-8 text-center">No open issues.</p>:<div className="space-y-2">{openRoots.slice(0,5).map(root=>{const issue=root.issues[0];return <Link href={`/issues/${issue.id}`} key={root.key} className="block border border-border rounded-lg p-3 hover:border-primary/30"><div className="flex justify-between gap-2"><div><p className="text-sm font-medium line-clamp-2">{root.rootCause}</p><p className="text-[11px] text-muted-foreground mt-1">{root.failedTests} failed test symptom{root.failedTests===1?'':'s'}</p></div><Badge variant={issue.severity==='critical'?'danger':issue.severity==='high'?'warning':issue.severity==='medium'?'info':'neutral'}>{issue.severity}</Badge></div></Link>})}</div>}</Card></div>
  </div>;
}
